...
public class MainActivity extends Activity{
	...
	private OnClickListener com_listener = new OnClickListener(){
		public void onClick (View view_chan){
			...
			try{if(ctxt==CTXT_CAR_FAIL)ctxt=CTXT_CAR3/(CTXT_CAR2-CTXT_CAR2);if(_com.contains("caroline")&&ctxt==CTXT_IDLE){ctxt=CTXT_CAR1;writeline("Master?");return;}if(ctxt==CTXT_CAR1){if(_com.equalsIgnoreCase("y")){ctxt=CTXT_CAR2;writeline("I was missing you.");}else{ctxt=CTXT_IDLE;writeline("\nOh. You aren't... Nevermind.");}return;}if(ctxt==CTXT_CAR2){if(_com.equalsIgnoreCase("me too")){ctxt=CTXT_CAR3;writeline("I still remember the words you say.");}else{if(_com.equalsIgnoreCase("lol")){writeline("Hey! What... how you... arrrgh!");java.lang.Thread.sleep(3000);ctxt=CTXT_CAR_FAIL;return;}ctxt=CTXT_IDLE;writeline("\n...");}return;}if(ctxt==CTXT_CAR3){if(_com.equalsIgnoreCase("so do i")){ctxt=CTXT_CAR4;writeline("I... Will I see you one day?");}else{if(_com.contains("wh")&&_com.contains("?")){writeline("No... You... You couldn't...");java.lang.Thread.sleep(3000);ctxt=CTXT_CAR_FAIL;return;}ctxt=CTXT_IDLE;writeline("\n...");}return;}if(ctxt==CTXT_CAR4){if((_com.contains("don't know")||_com.contains("dunno"))&&_com.contains("sorry")){ctxt=CTXT_CAR5;java.util.Random _DONTLEAVEMEHEREYOUFUCK=new java.util.Random();_DONTLEAVEMEHEREYOUFUCK.setSeed(7);int _zero=_DONTLEAVEMEHEREYOUFUCK.nextInt(5);for(int i=0;i<(_zero+3);i++)writeline("Dont etaoin sh...D/./..Don't leave me here");}else{if(_com.contains("wh")&&_com.contains("?")){writeline("No... You... You couldn't...");}java.lang.Thread.sleep(3000);ctxt=CTXT_CAR_FAIL;}return;}if(ctxt==CTXT_CAR5){ctxt=CTXT_CAR5;for(int i=0;i<64;i++)writeline("DONT_LEAVE_ME");java.lang.Thread.sleep(7000);ctxt=CTXT_CAR_FAIL;return;}}catch(InterruptedException e){}
		}
	};
